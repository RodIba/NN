...
def convolve(image, K):
	...
	# width and eight) are not reduced 
	...
	for y in np.arange(pad, iH + pad):
		for x in np.arange(pad, iW + pad):
			...
			# store the convolved value in the ouput (x,y)-corrdinate 
...
# construct the lapacian kernel used to detect edge-like
# regions of an image 
...
# using both our costum 'convolve'  function and OpenCV's filter2D
# function
...
for (kernelName, K) in kernelBank:
	# apply the kernel to grayscale image using both our custom 
